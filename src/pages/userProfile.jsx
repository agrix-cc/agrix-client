...
const UserProfile = () => {
    ...
    return (
        <div id="user-profile-page" className="min-h-screen bg-gray-50">
            ...
            <div id="profile-container" className="mx-auto max-w-screen-lg p-6 pt-16">
                ...
                <div id="user-profile" className="mb-6 flex items-center rounded-lg bg-white p-6 shadow-lg">
                    ...
                    <div id="user-details" className="ml-6 flex-1">
                        <h1 id="user-name" className="text-3xl font-semibold text-gray-800">{`${userData.first_name} ${userData.last_name}`}</h1>
                        <p id="profile-type" className="text-sm text-gray-500">{userData.profile_type}</p>
                        <p id="user-location" className="text-sm text-gray-400">{`${userData.city || ""} ${userData.district || ""}`}</p>
                        ...
                        <div id="message-connect-buttons" className="mt-4 flex space-x-4">
                            ...
                            {/* {connectionStatus === "accepted" ? (
                                <div className="relative">
                                    ...
                                    {openedRemoveBtn && (
                                        <div id="remove-connection-dropdown" className="absolute right-0 mt-2 rounded border bg-white shadow-lg">
                                            <button
                                                onClick={handleRemoveConnection}
                                                className="block w-full px-4 py-2 text-left text-red-600 transition-all hover:bg-gray-100"
                                            >
                                ...
                                <button id="pending-status-btn" className="rounded-full bg-yellow-500 px-6 py-2 text-white transition-all hover:bg-yellow-600">
                                    Pending
                                </button>
                            ) : (
                                <button
                ...
                {listings.length > 0 ? (
                    <div id="listings-grid" className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                        {listings.map((listing) => (
                            <Link to={`/product/${listing.id}`} key={listing.id}>
                                <div id={`listing-${listing.id}`} className="rounded-lg bg-white p-4 shadow transition-all hover:shadow-xl">
                                    <h3 className="text-lg font-semibold text-gray-800">{listing.title}</h3>
                                    <p className="text-sm text-gray-600">{listing.description}</p>
                                    <p className="mt-2 text-lg font-bold text-green-600">LKR {listing.price}</p>
                                </div>
            ...
            {/* Message UI Modal */}
            {showMessageUI && (
                <div id="message-modal" className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
